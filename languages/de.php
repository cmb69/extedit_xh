<?php

$plugin_tx['extedit']['mode_edit']="Bearbeiten";
$plugin_tx['extedit']['mode_view']="Ansicht";

$plugin_tx['extedit']['err_changed']="Der Inhalt von '%s' wurde in der Zwischenzeit geändert! Speichern Sie erneut, um die vorigen Änderungen zu überschreiben.";
$plugin_tx['extedit']['err_save']="Die Datei %s konnte nicht gespeichert werden.";
$plugin_tx['extedit']['err_unauthorized']="Sie sind nicht befugt diese Aktion durchzuführen!";

$plugin_tx['extedit']['imagepicker_title']="Bild auswählen";
$plugin_tx['extedit']['imagepicker_empty']="Es befinden sich keine Bilder im Bilderordner.";
$plugin_tx['extedit']['imagepicker_upload']="Hoch laden";
$plugin_tx['extedit']['imagepicker_dimensions']=" (%1\$d × %2\$d px)";
$plugin_tx['extedit']['imagepicker_err_1']="Die hoch geladene Datei überschreitet die upload_max_filesize Direktive in php.ini.";
$plugin_tx['extedit']['imagepicker_err_2']="Die hoch geladene Datei überschreitet die MAX_FILE_SIZE Direktive, die im HTML-Formular angegeben wurde.";
$plugin_tx['extedit']['imagepicker_err_3']="Die hoch geladene Datei wurde nur teilweise hoch geladen.";
$plugin_tx['extedit']['imagepicker_err_4']="Keine Datei wurde hoch geladen.";
$plugin_tx['extedit']['imagepicker_err_6']="Ein temporärer Ordner fehlt.";
$plugin_tx['extedit']['imagepicker_err_7']="Das Schreiben auf Disk schlug fehl.";
$plugin_tx['extedit']['imagepicker_err_8']="Eine PHP-Extension hat das Hochladen angehalten.";
$plugin_tx['extedit']['imagepicker_err_unknown']="Ein unbekannter Fehler trat während des Hochladens der Datei auf.";
$plugin_tx['extedit']['imagepicker_err_nofile']="Es wurde keine Datei zum Upload gewählt!";
$plugin_tx['extedit']['imagepicker_err_mimetype']="Die Datei ist kein Bild.";
$plugin_tx['extedit']['imagepicker_err_save']="Die hochgeladene Datei konnte nicht gespeichert werden!";

$plugin_tx['extedit']['synopsis']="Kurzfassung";
$plugin_tx['extedit']['synopsis_username']="%%BENUTZERNAME%%";
$plugin_tx['extedit']['synopsis_username_desc']="Der Name des Register_XH oder Memberpages_XH Benutzers, der den Inhalt bearbeiten darf.";
$plugin_tx['extedit']['synopsis_textname']="%%TEXTNAME%%";
$plugin_tx['extedit']['synopsis_textname_desc']="Der eindeutige Name des \"Extedits\". Lassen Sie diesen Parameter aus, um die Überschrift der enthaltenden Seite zu verwenden.";

$plugin_tx['extedit']['syscheck']="Systemprüfung";
$plugin_tx['extedit']['syscheck_phpversion']="PHP Version ist wenigstens %s";
$plugin_tx['extedit']['syscheck_phpversion_no']="PHP Version ist nicht wenigstens %s";
$plugin_tx['extedit']['syscheck_xhversion']="CMSimple_XH Version ist wenigstens %s";
$plugin_tx['extedit']['syscheck_xhversion_no']="CMSimple_XH Version ist nicht wenigstens %s";
$plugin_tx['extedit']['syscheck_writable']="Ordner '%s' ist beschreibbar";
$plugin_tx['extedit']['syscheck_writable_no']="Ordner '%s' ist nicht beschreibbar";

$plugin_tx['extedit']['cf_images_subfolder']="Soll der Editor nur Zugriff auf seinen eigenen Bilderordner haben?";
$plugin_tx['extedit']['cf_images_extensions']="Eine durch Komma getrennte Liste von erlaubten Dateinamen-Erweiterungen für den Upload.";
$plugin_tx['extedit']['cf_allow_scripting']="Sollen Plugin-Aufrufe in den \"extedits\" ausgewertet werden?";
$plugin_tx['extedit']['cf_allow_template']="Ob \"extedits\" im Template oder in Newsboxen eingebunden sind.";

?>
